...
function Todo() { 
  ...
  return (
    <>
        ...
        <div className="child-div text-center">
          <figure>
            <h5 className="subTitle">And The Task List Is Here 🕊</h5>
          </figure>
        </div>
  ...
  <div className="form-group row">
    ...
    <div className="addItems d-grid gap-2 col-6 mx-auto">
      ...
      <button type="button" className="btn btn-secondary" onClick={() => dispatch(addTask(inputTask, inputDesc), setInputTask(''),setInputDesc(''), confirm("Successfully Added"))}>
      ...
      <div className="showList my-2 list-group-item-primary ">
        {
          list.map((elem) => {
            ...
            <i class="fa-solid fa-trash-can fa-large my-2" title="Delete Item" onClick={()=>{dispatch(deleteTask(elem.id)), alert("Successfully Deleted")}}></i>
           </div>
            )  
          })
        }
